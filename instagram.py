 ######## THIS HORRIBLE (BUT WORKING) CODE WAS MADE BY TRUFOOX ########
...
with open("config.json", "r") as infile: # Starts config
    ...
    duplicatesAllowed = config["Duplicates_Allowed"]
    ...
    try:
        ...
        def get_random_image_url(api_url):
            ...
            while True:
                ...
                for sub in troublesome_subreddits or []: # 
                    currentTime = datetime.now().timestamp()
                    if currentTime - tSubreddits_times[troublesome_subreddits.index(sub)] > timeWait*5:
                        tSubreddits_times.remove(tSubreddits_times[troublesome_subreddits.index(sub)])
                        troublesome_subreddits.remove(sub)
                        print("Just removed troublesome subreddit from list after 5 cycles! lets goooo!!! Remove this line in the code now. Line 102. Also remove 103")
                        print()
                if debug == "True":
                ...
                if response.status_code == 200: # Test if request sucessful
                    if countattempt < timeout: # Ensures the code is not stuck in a loop
                        ...
                        if not chosenSubreddit in troublesome_subreddits:
                            if nsfwAllowed == "True" or not nsfw: # Makes sure not nsfw if nsfwAllowed is false
                                if image_shape(image_url):
                                    if not ".gif" in image_url: # Makes sure not .gif
                                        if not image_url in images and duplicatesAllowed == "False": # Makes sure not duplicate if duplicates disabled
                                            if not any(item.lower() in postCaption.lower() for item in blacklist): # Make sure caption doesnt contain blacklisted strings
                                                if "https://i.redd.it" in image_url: # Prevents weird image URLs (This fixes an error that occurs with non-reddit image URLs)
                                                    ...
                                                    with open("used_images.json", "w") as outfile: # Adds successful URL to previously used URLS list
                            ...
                            delete_previous_line()
                            print(Fore.RED + f"Subreddit has been flagged as overused  Trying again... x{countattempt}", flush=True)
                    else:
                        delete_previous_line()
                        print(Fore.RED + f"{datetime.now().strftime('%H:%M')} - Unable to find suitable url in r/{chosenSubreddit}. Skipping post. Subreddit has been logged as unsuitable for {timeWait*5} minutes", flush=True)
                        currentTime = datetime.now().timestamp() # Logs subreddit to be ignored for a while (prevents constant loops)
                        troublesome_subreddits.append(chosenSubreddit)
                        tSubreddits_times.append(currentTime)
                        print("")
                        return ""
                else:
                    delete_previous_line()
                    if response.status_code == 530:
                        print(Fore.RED + style.BOLD + f"{datetime.now().strftime('%H:%M')} - Failed. Cloudfare HTTP Status Code: 530 - The API this program utilizes appears to be under maintenence.\nThere is nothing that can be done to fix this but wait. Skipping attempt w/ +6 hour delay..." + style.END)
                        time.sleep(21600)
                    else:
        ...
        if not media_url == "": # Tests if there was a loop/error and does not continue if one occurred
            if defaultCaption == "False" or any(item.lower() in postCaption.lower() for item in captionCriteria): # Sets caption to either fallback or post caption
                upload_data = {
                    "image_url": media_url,
                    "caption": caption + '\n\n.' + '\n\n' + hashtags,
        ...
        print()
        print(style.BOLD + "ERROR INFO:" + Foree.RED, error)
        print()
        input(style.END + "Press Enter to exit...")
        exit()
